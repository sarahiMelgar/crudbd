...
ejecutaServicio(function () {
    $lista = select(pdo: Bd::pdo(), from: SOCIO, orderBy: SOC_NOMBRE);
    ...
    foreach ($lista as $modelo) {
        $encodeId = urlencode($modelo[SOC_ID]);
        $id = htmlentities($encodeId);
        $nombre = htmlentities($modelo[SOC_NOMBRE]);
        $apellidos = htmlentities($modelo[SOC_APELLIDOS]);
        $correo = htmlentities($modelo[SOC_CORREO]);
        ...
        $render .= "
            ...
            <dl class='col-12 my-3' style='border-top: 1px solid #f5c2c7;'></dl>";
    }
